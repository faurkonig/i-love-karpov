...
void MainWindow::on_pushButtonSaveText_clicked()
{
    ...
    for (int i = 0; i < arrSize; i++)
    {
        ...
        {
            // Если ячейка существует, то проверяем на корректность его значения
            jsonArr.append(ui->tableWidget->item(i, 0)->text().toInt(&okCell));
        }
        else
        {
            // Если ячейки не существует, то она создаётся
    ...
    {
        // Что-то не так со считыванием из таблицы
    }
...
void MainWindow::on_pushButtonOpenText_clicked()
{
    ...
    if (fileName.isEmpty())
    {
        QMessageBox::information(this, "Ошибка", "Ты еблан!"); // TODO Изменить
    }
    else
    {
        QFile file; file.setFileName(fileName);
        if (file.open(QIODevice::ReadOnly))
        {
            ...
            {
                QMessageBox::information(this, "Ошибка", "Ты еблан x2!"); // TODO Изменить
            }
            else
            {
                // Не ошибка
                if (jsonDoc.isArray())
                {
                    ...
                    if (len <= MAX_ARR)
                    {
                        ...
                        for (int i = 0; i < len; i++)
                        {
                            ...
                            {
                                // Если ячейка существует, то проверяем на корректность его значения
                                if (jsonArr[i].isDouble() && jsonArr[i].toInt() == jsonArr[i].toDouble())
                                {
                                    int value = jsonArr[i].toInt();
                    ...
                    {
                        QMessageBox::information(this, "Ошибка", "Ты еблан x3!"); // TODO Изменить
                    }
                }
            }
        }
        ...
        {
            // Не открыто
            QMessageBox::information(this, "Ошибка", "Файл тупой"); // TODO Изменить
        }
    }
}
...
//            jsonArr.append(ui->tableWidget->item(i, 0)->text().toInt(&okCell));
    ...
    {
        // Если хоть что-то с данными в таблице неправильно
    }
}
...
void MainWindow::on_pushButtonOpenBin_clicked()
{
    ...
    {
        QFile file; file.setFileName(fileName);
        if (file.open(QIODevice::ReadOnly) && file.bytesAvailable() % 4 == 0)
        {
            noAutoChange = false;
            ...
            while (!file.atEnd())
            {
                ...
                // Пока не прочитаем файл до конца
                int cellValue;
                file.read(mas, 4);
        ...
        {
            // Не открылся или неверное кол-во байт ;(
        }
    }
}
